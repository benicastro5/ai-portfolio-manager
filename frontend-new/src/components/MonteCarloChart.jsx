    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ background: 'var(--surface2)', borderRadius: '12px', padding: '18px', border: '1px solid var(--border)' }}>
        <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '4px' }}>
          Projected Portfolio Value — Growth Over Time
        </div>
        <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '14px' }}>
          Starting with {fmt(initial)}{monthly > 0 ? ` + ${fmt(monthly)}/month` : ''} · {(annReturn*100).toFixed(1)}% expected return · {(annVol*100).toFixed(1)}% volatility · compounded monthly
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--border)' }}>
                ...
                <th style={{ textAlign: 'right', padding: '8px 12px', color: '#dc2626', fontWeight: 700 }}>Worst (5%)</th>
                <th style={{ textAlign: 'right', padding: '8px 12px', color: '#f97316', fontWeight: 700 }}>Conservative (25%)</th>
                <th style={{ textAlign: 'right', padding: '8px 12px', color: '#3b82f6', fontWeight: 700 }}>Median</th>
                <th style={{ textAlign: 'right', padding: '8px 12px', color: '#22c55e', fontWeight: 700 }}>Optimistic (75%)</th>
                <th style={{ textAlign: 'right', padding: '8px 12px', color: '#16a34a', fontWeight: 700 }}>Best (95%)</th>
              </tr>
            </thead>
            <tbody>
              {milestones.map((m, i) => (
                <tr key={m.yr} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--surface)' }}>
                  ...
                  {monthly > 0 && (
                    <td style={{ padding: '10px 12px', textAlign: 'right', color: 'var(--text-muted)', fontSize: '12px' }}>{fmt(m.totalContrib)}</td>
                  )}
        ...
        {monthly > 0 && (
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '10px' }}>
            "Total Invested" = initial deposit + all monthly contributions. Difference between Total Invested and Median = investment returns from compounding.
          </div>
        )}
      </div>
        ...
        <ResponsiveContainer width="100%" height={320}>
          <AreaChart data={chartData} margin={{ top: 8, right: 20, left: 10, bottom: 0 }}>
            <XAxis dataKey="year" type="number" domain={[0, 'dataMax']}
              ticks={Array.from({ length: Math.ceil(horizonYears) + 1 }, (_, i) => i).filter(i => i <= horizonYears)}
              tickFormatter={v => `${v}y`} tick={{ fontSize: 11 }} />
